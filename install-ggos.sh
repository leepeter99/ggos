...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 GGOS Installation Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 GGOS Installation Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
print_header "⚠️  CRITICAL WARNING - Existing GGOS Detected"
...
if [ -d "ggos" ]; then
  ...
  echo -e "${RED}║                                                                       ║${NC}"
  echo -e "${RED}║  An existing GGOS installation was detected at ~/ggos           ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
  ...
  echo -e "${GREEN}ggos exists, backing up to .config/ggos-backups folder.${NC}"
  if [ -d ".config/ggos-backups" ]; then
    echo -e "${GREEN}Moving current version of GGOS to backups folder.${NC}"
    mv "$HOME"/ggos .config/ggos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving GGOS to it.${NC}"
    mkdir -p .config/ggos-backups
    mv "$HOME"/ggos .config/ggos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing GGOS.${NC}"
