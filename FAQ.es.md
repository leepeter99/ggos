...
simplificar la gestión de tu entorno `ggos`. Proporciona un conjunto completo
...
- Edita `~/ggos/hosts/<HOSTNAME>/variables.nix` y descomenta `mimeDefaultApps`.
...
<summary>**❄ ¿Por qué creaste GGOS? **</summary>
...
- El nombre `GGOS` es un chiste interno entre amigos.
...
- Crea un fork de GGOS, luego modifícalo.
- Si encuentras un issue y lo arreglas, o proporcionas una nueva característica,
  por favor compártelo.
- GGOS no es una distro. En este momento no hay planes para crear un ISO de
  instalación.
...
- Edita `~/ggos/modules/core/flatpak.nix`
...
- Ve al directorio `~/ggos/host/HOSTNAME`
...
1. En el archivo, `~/ggos/modules/core/system.nix`
...
<details>
<summary>**¿Cómo agrego aplicaciones a GGOS? **</summary>
...
1. Para aplicaciones que se incluirán en todos los hosts definidos edita el
   archivo `~/ggos/modules/core/packages.nix` file.

Hay una sección que comienza con: `environment.systemPackages = with pkgs;`

Seguida de una lista de paquetes. Estos son requeridos para GGOS.
...
<details>
<summary>** Hice un cambio a mi configuración de GGOS, ¿cómo lo activo? **</summary>

- Usa la utilidad `zcli`. `zcli rebuild`
- El alias legacy `fr`, Flake Rebuild está depreciado pero aún disponible **
  NOTA: Si **creaste un nuevo archivo\*\*
- necesitarás ejecutar un comando `git add .` en la carpeta `ggos`
...
1. Tienes que editar el archivo `hardware.nix` para ese host en
   `~/ggos/hosts/HOSTNAME/hardware.nix` y override el default.
...
2. Edita el archivo `~/ggos/flake.nix`. Cambia la línea:

- `host = "NEW-HOSTNAME"`

3. En el directorio `~/ggos` ejecuta `git add .` _El rebuild fallará con un
...
1. Edita el archivo `~/ggos/modules/core/boot.nix` file.
2. Busca:
...
1. Ejecuta el script `install-ggos.sh` y selecciona el template
   `nvidia-laptop` o si configurando manualmente, establece el template en el
   `flake.nix` a `nvidia-prime`

2. En el archivo `~/ggos/hosts/HYBRID-HOST/variables.nix` necesitarás
...
4. En el archivo `~/ggos/modules/home/hyprland/config.nix` hay un ENV
...
1. Edita el archivo `~/ggos/modules/core/stylix.nix` file.
...
1. Edita el archivo `~/ggos/hosts/HOSTNAME/varibles.nix`
2. Cambia el `stylixImage =` al nombre del archivo que quieres usar. Los
   wallpapers están en `~/ggos/wallpapers`
...
- Los wallpapers se almacenan en el directorio `~/ggos/wallpapers`.
...
1. Edita el archivo `~/ggos/modules/home/hyprland/config.nix` file.
...
1. Edita el archivo `~/ggos/modules/home/scripts/wallsetter`
...
<details>
<summary>**⬆ ¿Cómo actualizo GGOS?  **</summary>
...
Altamente recomendado: Lee `GGOS-Upgrade.md` antes de proceder. Detalla el
upgrade automatizado seguro, respaldo, y proceso de revert.

Usa el upgrade automatizado v2.3 → v2.4. Ve `GGOS-Upgrade.md` y
...
1. Primero respaldo tu directorio `ggos` existente. ej.
   `cp -r ~/ggos ~/ggos-backup`

2. No hay update directo. Cuando clones la nueva config los archivos de config y
   layout han cambiado.

3. Necesitas instalar ggos como una nueva install. `./install-ggos.sh`
...
2. Respalda tu directorio `ggos` ej. `cp -r ~/ggos ~/ggos-backup`

3. Ejecuta el script `./install-ggos.sh` y sigue las nuevas instrucciones de
   install.

</details>

<details>
<summary> ¿Cómo sé cuándo se lanza una nueva versión de GGOS? </summary>
...
<details><summary>**📂 Layout de GGOS v2.3**</summary>
...
~/ggos/
    ...
    ├── flake.nix                   # flake que controla config GGOS
    ├── install-ggos.sh          # Script de install para GGOS
    ├── LICENSE                     # Licencia MIT que usa GGOS
    └── README.md                   # Documento introductorio para GGOS
```
...
`~/ggos/modules/home/yazi.nix`

Yazi está configurado como VIM y motions VIM

El keymap está en el archivo `~/ggos/modules/home/yazi/keymap.toml` file
...
Los bindings de kitty están configurados en `~/ggos/modules/home/kitty.nix`
...
Edita el `/ggos/modules/home/wezterm.nix` Cambia `enable = false` a
`enable = true;`\
Guarda el archivo y rebuild ggos con el comando `zcli rebild` o el alias `fr`
...
Los bindings de kitty están configurados en `~/ggos/modules/home/wezterm.nix`
...
1. Edita el archivo `~/ggos/modules/home/ghostty.nix` file.
