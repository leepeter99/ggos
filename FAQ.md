...
your `ggos` environment. It provides a comprehensive set of commands to
...
GGOS CLI Utility -- version 1.0.2
...
<summary>**❄ Why did you create GGOS ? **</summary>
...
- The `GGOS` name is an inside joke among friends.
...
- You create a fork of GGOS, then modify it.
- If you find an issue and fix it, or provide a new feature, please share it.
- GGOS is not a distro. At this time there are no plans to create an install
  ISO.
...
- Edit `~/ggos/hosts/<HOSTNAME>/variables.nix` and uncomment `mimeDefaultApps`.
...
- Edit `~/ggos/modules/core/flatpak.nix`
...
1. In the file, `~/ggos/modules/core/system.nix`
...
Monitor settings are in the file: `~/ggos/hosts/<HOSTNAME>/variables.nix`
...
<img align="center" width="90%" src="img/nwg-displays.png" />
...
<details>
<summary>**How do I add applications to GGOS? **</summary>
...
1. For applications to be included in all defined hosts edit the
   `~/ggos/modules/core/packages.nix` file.

There is a section that begins with: `environment.systemPackages = with pkgs;`

Followed by a list of packages These are required for GGOS.
...
`~/ggos/hosts/<HOSTNAME>/host-packages.nix`
...
<details>
<summary>** I made a change to my GGOS configuration, how do I activate it? **</summary>

- Use the `zcli` utility. `zcli rebuild`
- The legacy `fr`, Flake Rebuild alias, is depreciated but still available **
  NOTE: If you **created a new file\*\*
- you will need to run a `git add .` command in the `ggos` folder
...
1. You have to edit the `hardware.nix` file for that host in
   `~/ggos/hosts/HOSTNAME/hardware.nix` and override the default.
...
- `cp -rpv ~/ggos/hosts/OLD-HOSTNAME ~/ggos/hosts/NEW-HOSTNAME`

2. Edit the `~/ggos/flake.nix` file. Change the line:

- `host = "NEW-HOSTNAME"`

3. In the `~/ggos` Directory run `git add .` _The rebuild will fail with a
...
1. Edit the `~/ggos/modules/core/boot.nix` file.
2. Look for:
...
1. Either run the `install-ggos.sh` script and select `nvidia-laptop`
   template or if configuring manually, set the template in the `flake.nix` to
   `nvidia-prime`

2. In the `~/ggos/hosts/HYBRID-HOST/variables.nix` file you will need to set
...
4. In the `~/ggos/modules/home/hyprland/config.nix` file is an ENV
...
1. Edit the `~/ggos/modules/core/stylix.nix` file.
...
1. Edit the `~/ggos/hosts/HOSTNAME/varibles.nix`
...
- Wallpapers are stored in the `~/ggos/wallpapers` directory.
...
1. Edit the `~/ggos/modules/home/hyprland/config.nix` file.
...
1. Edit the `~/ggos/modules/home/scripts/wallsetter`
...
<details>
<summary>**⬆ How do I update GGOS?  **</summary>
...
Strongly recommended: Read `GGOS-Upgrade.md` before proceeding. It details
the safe, automated upgrade, backup, and revert process.

Use the automated v2.3 → v2.4 upgrade. See `GGOS-Upgrade.md` and
...
```bash
git -C ~/ggos fetch origin
git -C ~/ggos show origin/main:upgrade-2.3-to-2.4.sh > ~/upgrade-2.3-to-2.4.sh
...
1. First backup your existing `ggos` directory. e.g.
   `cp -r ~/ggos ~/ggos-backup`

2. There is no direct update. When you clone the the new config the config files
   and layout have changed.

3. You need to install ggos like a new install. `./install-ggos.sh`
...
2. Backup your `ggos` directory e.g. `cp -r ~/ggos ~/ggos-backup`

3. Run the `./install-ggos.sh` script and follow the new install
   instructions.

</details>

<details>
<summary> How do I know when a new version of GGOS is released? </summary>
...
<details><summary>**📂 GGOS v2.3 Layout**</summary>
...
~/ggos/
    ...
    ├── flake.nix                   # flake that controls GGOS config
    ├── install-ggos.sh          # Install script for GGOS
    ├── LICENSE                     # MIT license GGOS is using
    └── README.md                   # Intro document for GGOS
```
...
- If you find other files that cause this you can add them in the
  `ggos/modules/home/scripts/default.nix`
  <br>
...
The Yazi configuration file is located in `~/ggos/modules/home/yazi.nix`

Yazi is configured like VIM and VIM motions

The keymap is in the `~/ggos/modules/home/yazi/keymap.toml` file
...
- That feature is called "cursor_trail" in the
  `~/ggos/modules/home/kitty.nix` file.
...
The kitty bindings are configured in `~/ggos/modules/home/kitty.nix`
...
Edit the `/ggos/modules/home/wezterm.nix` Change `enable = false` to
`enable = true;`\
Save the file and rebuild ggos with the `fr` command.
...
The kitty bindings are configured in `~/ggos/modules/home/wezterm.nix`
...
1. Edit the `~/ggos/modules/home/ghostty.nix` file.
...
- Use the command `zcli rebuild` or `fr` Flake Rebuild alias. If you **created a
  ...
  `ggos` folder. If successful, a new generation will be generated with your
...
Edit the `/ggos/modules/home/wezterm.nix` Change `enable = false` to
`enable = true;`\
Save the file and rebuild `ggos` with the `zcli rebild` command or the `fr`
alias
