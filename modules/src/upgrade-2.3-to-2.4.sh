...
# Define directories
GGOS_DIR="$HOME/ggos"
BACKUP_BASE_DIR="$HOME/.config/ggos-backups"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
BACKUP_DIR="$BACKUP_BASE_DIR/ggos-2.3-upgrade-backup-$TIMESTAMP"

# Target branch for 2.4 release (override by exporting BRANCH)
BRANCH="${BRANCH:-stable-2.4}"

# Define log file
LOG_FILE="$HOME/ggos-upgrade-$TIMESTAMP.log"
...
create_backup() {
    ...
    if [ ! -d "$GGOS_DIR" ]; then
        print_error "GGOS directory not found at $GGOS_DIR"
        return 1
    fi
    ...
    print_info "Copying entire ggos directory..."
    if cp -r "$GGOS_DIR" "$BACKUP_DIR/"; then
        print_success "Backup created successfully at: $BACKUP_DIR"
        echo -e "${GREEN}📁 Backup location: $BACKUP_DIR/ggos${NC}"
        return 0
    else
...
revert_from_backup() {
    ...
    if [ ! -d "$BACKUP_DIR/ggos" ]; then
        print_error "Backup not found at $BACKUP_DIR/ggos"
        echo -e "${YELLOW}Available backups:${NC}"
        ls -la "$BACKUP_BASE_DIR/" 2>/dev/null || echo "No backups found"
        return 1
    fi
    
    print_warning "This will completely restore your GGOS 2.3 configuration"
    echo -e "${YELLOW}Current ggos directory will be replaced with backup${NC}"
    ...
    print_info "Removing current ggos directory..."
    if [ -d "$GGOS_DIR" ]; then
        rm -rf "$GGOS_DIR"
    fi
    ...
    if cp -r "$BACKUP_DIR/ggos" "$HOME/"; then
        print_success "Successfully reverted to GGOS 2.3 backup"
        
        # Change to the restored directory
        cd "$GGOS_DIR" || return 1
        ...
        if command -v nh &> /dev/null; then
            nh os boot ~/ggos --hostname "$profile"
        else
            sudo nixos-rebuild boot --flake ~/ggos/#"$profile"
        fi
        
        if [ $? -eq 0 ]; then
            print_success "System successfully reverted to GGOS 2.3"
...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 GGOS 2.4 Upgrade Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
  echo -e "${GREEN}║   🎉 Your system has been upgraded to GGOS 2.4                    ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 GGOS 2.4 Upgrade Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
switch_to_main_handling_dirty() {
  ...
  print_success "Successfully switched to GGOS 2.4 ($BRANCH branch)"
}
...
print_header "GGOS 2.3 → 2.4 Upgrade Script"

echo -e "${CYAN}🚀 Welcome to the GGOS 2.4 Upgrade Script!${NC}"
echo ""
echo -e "${YELLOW}📋 This script will:${NC}"
echo -e "  • Create a complete backup of your current system"
echo -e "  • Verify you're currently running GGOS 2.3"
echo -e "  • Upgrade to GGOS 2.4 from $BRANCH"
...
# Change to ggos directory
if [ ! -d "$GGOS_DIR" ]; then
    print_error "GGOS directory not found at $GGOS_DIR"
    print_error "Please ensure GGOS is installed at ~/ggos"
    exit 1
fi
...
# Check if we're in a GGOS directory
if [ ! -f "./flake.nix" ] || [ ! -d "./hosts" ]; then
  print_error "This doesn't appear to be a GGOS directory."
  print_error "Please ensure you're in the correct directory."
  exit 1
fi

print_success "GGOS directory structure detected"
...
if grep -q "displayManager" ./hosts/default/variables.nix; then
  ...
  print_success "Verified: Template appears to be GGOS 2.3-style"
fi

# Comprehensive pre-check analysis
perform_precheck_analysis() {
    local analysis_file="$HOME/ggos-upgrade-analysis-$TIMESTAMP.txt"
    
    print_header "Pre-Upgrade Analysis"
    print_info "Analyzing your current GGOS configuration..."
    print_info "Analysis report will be saved to: $analysis_file"
    
    {
        echo "============================================"
        echo "  GGOS 2.3 → 2.4 Upgrade Analysis Report"
        echo "  Generated: $(date)"
        ...
        if [ -d ".git" ]; then
            ...
            if git remote get-url origin >/dev/null 2>&1; then
                ...
                if [[ "$remote_url" != *"leepeter99/ggos"* ]]; then
                    ...
                    echo "     ./compare-ggos-config.sh -r $remote_url -b $current_branch"
                fi
            fi
            ...
            echo "     git init && git add . && git commit -m 'Initial GGOS configuration'"
        fi
        echo ""
        ...
        echo "   ~/.config/ggos-backups/ggos-2.3-upgrade-backup-$TIMESTAMP/"
        echo ""
...
echo ""
prompt_yes "Continue with the upgrade to GGOS 2.4? (Y/N):"
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
print_header "Fetching GGOS 2.4"
...
HOST_DIRS=()
if [ -d "$BACKUP_DIR/ggos/hosts" ]; then
    for dir in "$BACKUP_DIR/ggos/hosts"/*/; do
        [ -d "$dir" ] || continue
        hostname=$(basename "$dir")
        if [ "$hostname" != "default" ]; then
            HOST_DIRS+=("$hostname")
...
merge_variables() {
    local hostname=$1
    local old_vars_file="$BACKUP_DIR/ggos/hosts/$hostname/variables.nix"
    ...
    if [ -f "$BACKUP_DIR/ggos/hosts/$hostname/hardware.nix" ]; then
        cp "$BACKUP_DIR/ggos/hosts/$hostname/hardware.nix" "./hosts/$hostname/"
        print_success "✓ Preserved hardware.nix for host: $hostname"
    fi
    
    # Also copy host-packages.nix if it exists (user's custom packages)
    if [ -f "$BACKUP_DIR/ggos/hosts/$hostname/host-packages.nix" ]; then
        cp "$BACKUP_DIR/ggos/hosts/$hostname/host-packages.nix" "./hosts/$hostname/"
    ...
    if [ -f "$BACKUP_DIR/ggos/hosts/$hostname/default.nix" ]; then
        # Check if it's different from the template default.nix
        if ! cmp -s "$BACKUP_DIR/ggos/hosts/$hostname/default.nix" "./hosts/default/default.nix"; then
            cp "$BACKUP_DIR/ggos/hosts/$hostname/default.nix" "./hosts/$hostname/"
...
if [ -f "$BACKUP_DIR/ggos/modules/core/packages.nix" ]; then
    ...
    if ! cmp -s "$BACKUP_DIR/ggos/modules/core/packages.nix" "./modules/core/packages.nix"; then
        ...
        cp "$BACKUP_DIR/ggos/modules/core/packages.nix" "./modules/core/packages.nix"
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            print_info "New packages available in GGOS 2.4:"
...
update_flake_configuration() {
    ...
    if [ -n "${HOST_DIRS[0]}" ] && [ -f "$BACKUP_DIR/ggos/hosts/${HOST_DIRS[0]}/variables.nix" ]; then
        detected_username=$(grep 'gitUsername' "$BACKUP_DIR/ggos/hosts/${HOST_DIRS[0]}/variables.nix" | sed 's/.*= *"\([^"]*\)".*/\1/' | head -1)
    fi
...
print_header "Building GGOS 2.4"
...
# Build the system
echo ""
prompt_yes "Ready to build GGOS 2.4? This may take several minutes. (Y/N):"
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ "$USE_NH" = true ]; then
    ...
    if nh os boot ~/ggos --hostname "$GPU_PROFILE"; then
        BUILD_SUCCESS=true
    fi
else
    print_info "Using nixos-rebuild for build..."
    if sudo nixos-rebuild boot --flake ~/ggos/#"$GPU_PROFILE"; then
        BUILD_SUCCESS=true
    fi
fi
...
if [ "$BUILD_SUCCESS" = true ]; then
    ...
    echo ""
    echo -e "${CYAN}🔧 Key Changes in GGOS 2.4:${NC}"
    ...
    prompt_yes "Would you like to automatically revert to GGOS 2.3? (Y/N):"
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        revert_from_backup
    else
