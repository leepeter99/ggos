...
# GGOS 2.3 → 2.4 Upgrade Guide

This guide provides safe upgrade scripts to migrate from GGOS 2.3 to 2.4, handling all the breaking changes and ensuring your custom configuration is preserved.
...
3. **Version Verification**: Confirms you're running GGOS 2.3
...
- **Complete Backup**: Full copy of your ggos directory before any changes
...
```bash
# You can run the script from anywhere; it operates on ~/ggos
...
- **Generate a detailed report** saved as `~/ggos-upgrade-analysis-TIMESTAMP.txt`
...
- Download GGOS 2.4 from main branch
...
```
~/.config/ggos-backups/ggos-2.3-upgrade-backup-TIMESTAMP/
```
...
1. Remove current ggos directory: `rm -rf ~/ggos`
2. Restore from backup: `cp -r ~/.config/ggos-backups/[backup-name]/ggos ~/`
3. Rebuild system: `nh os boot ~/ggos --hostname [profile]`
4. Reboot system
...
- Check if your preferred terminal is enabled in `~/ggos/hosts/[hostname]/variables.nix`
...
### Log File Location
Each run creates a log file at: `~/ggos-upgrade-TIMESTAMP.log`

## 📁 File Structure After Upgrade

Your host configuration will be updated with new 2.4 structure:
```
~/ggos/hosts/[your-hostname]/
