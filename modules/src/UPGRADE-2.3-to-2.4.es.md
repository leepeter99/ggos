...
# Guía de Actualización GGOS 2.3 → 2.4

Esta guía proporciona scripts de actualización seguros para migrar de GGOS 2.3 a 2.4, manejando cambios incompatibles y preservando tu configuración personalizada.
...
- **Respaldo completo** de ~/ggos antes de cambios
...
```bash
# Puedes ejecutarlo desde cualquier ruta; opera sobre ~/ggos
...
- **Genera un informe detallado** en `~/ggos-upgrade-analysis-TIMESTAMP.txt`
...
1. Elimina `~/ggos`: `rm -rf ~/ggos`
2. Restaura desde el respaldo: `cp -r ~/.config/ggos-backups/[backup-name]/ggos ~/`
3. Rebuild: `nh os boot ~/ggos --hostname [profile]`
4. Reinicia
...
- Verifica tu terminal preferida en `~/ggos/hosts/[hostname]/variables.nix`
- Confirma que navegador y otras apps sigan disponibles

### Log de ejecución
Cada corrida genera un log en: `~/ggos-upgrade-TIMESTAMP.log`

## 📁 Estructura tras la actualización

```
~/ggos/hosts/[tu-hostname]/
