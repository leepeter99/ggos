...
##########################################
# GGOS Configuration Comparison Tool
# Author: Don Williams
# Date: $(date +"%B %d, %Y")
##########################################
...
# Define directories
GGOS_DIR="$HOME/ggos"
TEMP_DIR="/tmp/ggos-comparison-$$"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
REPORT_FILE="$HOME/ggos-config-comparison-$TIMESTAMP.txt"

# Default comparison targets
DEFAULT_REPO="https://github.com/leepeter99/ggos.git"
DEFAULT_BRANCH="main"
...
show_usage() {
  echo -e "${CYAN}GGOS Configuration Comparison Tool${NC}"
  echo ""
  echo "Usage: $0 [OPTIONS]"
  echo ""
  echo "Options:"
  echo "  -r, --repo URL          Remote repository URL to compare against"
  echo "                          (default: https://github.com/leepeter99/ggos.git)"
  ...
  echo "  $0 -r https://github.com/user/ggos.git -b dev"
...
perform_comparison() {
  print_header "GGOS Configuration Comparison Tool"
  ...
  if [ ! -d ".git" ]; then
    print_warning "Current GGOS directory is not a git repository"
    print_info "This tool works best when your GGOS is version controlled"
  fi
  ...
  {
    echo "============================================"
    echo "  GGOS Configuration Comparison Report"
    echo "  Generated: $(date)"
    echo "  Local: $GGOS_DIR"
    ...
    if [ -d ".git" ]; then
      ...
      echo "   git init"
      echo "   git add ."
      echo "   git commit -m 'Initial GGOS configuration'"
    fi
    echo ""
    ...
    echo "🔄 FOR UPGRADING TO GGOS 2.4:"
