[English](GGOS-Upgrade.md) | [Español](GGOS-Upgrade.es.md)

# 🚀 GGOS Upgrade Procedure
...
1. **Ensure your current GGOS is up-to-date:**
   - Navigate to your GGOS directory:
     ```bash
     cd ~/ggos
     ```
...
2. **Backup your current GGOS directory:**
   ```bash
   mv ~/ggos ~/ggos-backup
   ```
...
3. **Clone the v2.4 branch:**
   ```bash
   git clone https://github.com/leepeter99/ggos.git -b Stable-2.4 --depth=1
   cd ~/ggos
   ```
...
GGOS v2.4 introduces new features in the host configuration, giving you more
...
- **Migration Steps:**
  ...
  1. **Copy the `default` host template** to a new directory named after your
     ...
     cp -r ~/ggos/hosts/default ~/ggos/hosts/YOURHOSTNAME
     ```
     _Example:_ If your hostname is `nixos`:
     ```bash
     cp -r ~/ggos/hosts/default ~/ggos/hosts/nixos
     ```

  2. **Copy the hardware configuration** from your backup:
     ```bash
     # Replace nixos with your hostname if different
     cp ~/ggos-backup/hosts/nixos/hardware.nix ~/ggos/hosts/nixos/hardware.nix
     ```

  3. ❗ **IMPORTANT:** In the `ggos` directory, stage your changes:
     ```bash
     git add .
     ```

  4. **Manually edit your new host files** (`~/ggos/hosts/YOURHOSTNAME`) to
...
2. **Run a configuration check** from the `ggos` directory:
   ```bash
   nix flake check
   ```
...
1. **Execute the rebuild command** from the `ggos` directory:
   ```bash
