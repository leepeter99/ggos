{
  ...
  in {
    ...
    devShells.${system} = {
      default = import ./dev-shell.nix {pkgs = nixpkgs.legacyPackages.${system};};
      minimal = nixpkgs.legacyPackages.${system}.mkShell {
        buildInputs = with nixpkgs.legacyPackages.${system}; [git nixfmt-rfc-style];
      };
    };
